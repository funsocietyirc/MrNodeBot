...
class IrcWrappers {
     ...
     */
    async handleAction(from, to, text, message) {
        ...
        for (const command of this.app.OnAction.values()) {
            try {
     ...
     * @param {object} message - IRC information such as user, and host
     */
    async handleOnKick(channel, nick, by, reason, message) {
        ...
        for (const command of this.app.OnKick.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
     ...
     * @param {object} message - IRC information such as user, and host
     */
    async handleNickChanges(oldNick, newNick, channels, message) {
        ...
        // Run events
        for (const command of this.app.NickChanges.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleOnNotice(from, to, text, message) {
        // Do not handle our own actions, or those on the ignore list
        if (from === this.app.nick || _.includes(this.app.Ignore, _.toLower(from))) return;

        for (const command of this.app.OnNotice.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleOnJoin(channel, nick, message) {
        // Handle Ignore
        if (_.includes(this.app.Ignore, _.toLower(nick))) return;
        ...
        for (const command of this.app.OnJoin.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleOnPart(channel, nick, reason, message) {
        const normalizedReason = IrcWrappers._normalizeText(reason);
        // Handle Ignore
        if (_.includes(this.app.Ignore, _.toLower(nick))) return;
        if (nick === this.app.nick) {
        ...
        for (const command of this.app.OnPart.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleOnQuit(nick, reason, channels, message) {
        const normalizedReason = IrcWrappers._normalizeText(reason);
        //  Handle Ignore
        if (_.includes(this.app.Ignore, _.toLower(nick))) return;
        ...
        for (const command of this.app.OnQuit.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleOnTopic(channel, topic, nick, message) {
        ...
        for (const command of this.app.OnTopic.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
     ...
     * @param {object} message IRC information such as user, and host
     */
    async handleCtcpCommands(from, to, text, type, message) {
        const normalizedText = IrcWrappers._normalizeText(text);
        ...
        for (const command of this.app.OnCtcp.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
     ...
     */
    async handleRegistered(message) {
        logger.info(t('events.registeredToIrc'));
        for (const command of this.app.Registered.values()) {
            try {
                // Is the callback a promise?
                const isPromise = helpers.isAsync(command.call);
                // Call Function
    ...
    async handleCommands(from, to, text, message) {
        ...
        if (!is.triggered && !is.ignored && !is.self) {
            for (const command of this.app.Listeners.values()) {
                try {
                    // Is the callback a promise?
                    const isPromise = helpers.isAsync(command.call);
                    // Call Function
        ...
        // The following administration levels piggy back on services, thus we check the acc status of the account and defer
            ...
            // Old Freenode services style
            // this.app.say(`${this.app.Config.nickserv.nick}${first}`, `status ${from}`);
            // New Libera services style
            this.app.say(`${this.app.Config.nickserv.nick}${first}`, `info =${from}`);
        }
        // Invalid Command
        else {
            this.app.say(from, t('errors.invalidCommand', {
    ...
    async handleAuthenticatedCommands(nick, to, text, message) {
        ...
        // Parse vars
        // STATUS IronY 3 IronY
        // const [user, acc, code] = normalizedText.replace('STATUS', '').split(' ');
        // This is tied to FREENODE
        // OLD:
        // const nickservMatches = normalizedText.match(/STATUS (\w+) (\d) (\w+)/);
        const nickservStatusMatches = normalizedText.match(/STATUS (\w+) (\d) (\w+)/);

        if (_.has(nickservStatusMatches, 'length') && nickservStatusMatches.length >= 3) {
            user = nickservStatusMatches[1];
            identified = nickservStatusMatches[2] === this.app.Config.nickserv.accCode;
        }

        // OLD:
        //if (!_.has(nickservMatches, 'length') || nickservMatches.length < 3) return false;

        // Libera.Chat/Atheme style NickServ INFO =nick response for online users.
        const nickservInfoMatches = normalizedText.match(/Information on (\S+) \(account [^)]+\):/);

        if (_.has(nickservInfoMatches, 'length') && nickservInfoMatches.length >= 2) {
            user = nickservInfoMatches[1];
            identified = true;
        }

        // OLD:
        //const user = nickservMatches[1];
        //const code = nickservMatches[2];
        if (!user || _.isUndefined(identified)) return false;
        ...
        for (const k of [...this.app.AdmCallbacks]) {
            const [i, v] = k;
            // Short circuit
        ...
        admCall.is.identified = identified;
