    ...
    async handleCommands(from, to, text, message) {
        ...
        if (matchedInvalidCommand) {
            ...
            const bestGuess = await new Promise((res, rej) => {
                // TODO hoist this up so it is no dynamic on every command
                const cmdArr = _.map(Array.from(this.app.Commands.keys()), x => ({cmd: x}));
                nn.findMostSimilar({cmd: cmd}, cmdArr, [
            ...
            // We have a likely guess
            if (!_.isEmpty(bestGuess) && bestGuess.probability && bestGuess.nearestNeighbor && bestGuess.nearestNeighbor.cmd && bestGuess.probability >= 0.75) {
                this.app.say(to, `I am sorry ${from}, ${cmd} is an invalid option. Perhaps you meant "${bestGuess.nearestNeighbor.cmd}"?`);
            }
            // No idea
            else {
