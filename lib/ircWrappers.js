...
class IrcWrappers {
    ...
    async handleCommands(from, to, text, message) {
        ...
        const trigger = (_.isUndefined(this.app.Config.bot.trigger) || _.isEmpty(this.app.Config.bot.trigger)) ? this.app._ircClient.nick : this.app.Config.bot.trigger;
        ...
        const cmd = triggerSpace ? (is.privateMsg ? textArray[0] : textArray[1]) : textArray[0].replace(trigger, '');
        textArray.splice(0, triggerSpace ? (is.privateMsg ? 1 : 2) : 1);
        const output = textArray.join(' ');

        const triggerMatched = normalizedText.startsWith(trigger);
