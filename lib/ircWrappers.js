...
class IrcWrappers {
    ...
    handleAction(from, to, text, message) {
        ...
        // This is from a channel, and we have a MQTT server, broadcast
        if (this.app._ircClient.isChannel(to) && this.app._MQTTserver) {
            this.app._MQTTserver.publish({
                topic: `/irc/channels/${from}/action`,
    ...
    handleOnKick(channel, nick, by, reason, message) {
        ...
        // This is from a channel, and we have a MQTT server, broadcast
        if (this.app._MQTTserver) {
            this.app._MQTTserver.publish({
                topic: `/irc/channels/${channel}/kick`,
                payload: JSON.stringify({
                    channel,
                    nick,
                    by,
    ...
    handleOnNotice(from, to, text, message) {
        ...
        if (this.app._ircClient.isChannel(to) && this.app._MQTTserver) {
            this.app._MQTTserver.publish({
                topic: `/irc/channels/${from}/notice`,
                payload: JSON.stringify({
                    from,
                    to,
                    text,
    ...
    handleOnTopic(channel, topic, nick, message) {
        ...
        if (this.app._MQTTserver) {
            this.app._MQTTserver.publish({
                topic: `/irc/channels/${channel}/topic`,
                payload: JSON.stringify({
                    channel,
                    topic,
                    nick,
    ...
    async handleCommands(from, to, text, message) {
        ...
        if (this.app._ircClient.isChannel(to) && this.app._MQTTserver) {
            this.app._MQTTserver.publish({
                topic: `/irc/channels/${from}/message`,
                payload: JSON.stringify({
                    from,
                    to,
                    text,
