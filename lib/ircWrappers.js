...
class IrcWrappers {
    ...
    handleAction(from, to, text, message) {
        ...
        this.app.OnAction.forEach((value, key) => {
            try {
                value.call.bind(this.app, from, to, normalizedText, message)();
            } catch (err) {
    ...
    handleOnKick(channel, nick, by, reason, message) {
        ...
        if (nick === this.app.nick) logger.info(t('events.kickLoggingBy', {
            channel,
            by,
            reason
        }));

        if (by === this.app.nick) logger.info(t('events.kickLoggingFrom', {
            nick,
            channel,
            normalizedReason
        }));

        this.app.OnKick.forEach((value, key) => {
            try {
                value.call.bind(this.app, channel, nick, by, normalizedReason, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'onKick'
                }), err);
    ...
    handleNickChanges(oldNick, newNick, channels, message) {
        ...
        // Run events
        this.app.NickChanges.forEach((value, key) => {
            try {
                value.call.bind(this.app, oldNick, newNick, channels, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'nickChange'
                }), err);
    ...
    handleOnNotice(from, to, text, message) {
        ...
        this.app.OnNotice.forEach((value, key) => {
            try {
                value.call.bind(this.app, from, to, text, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'onNotice'
                }), err);
    ...
    handleOnJoin(channel, nick, message) {
        ...
        if (nick === this.app.nick) logger.info(t('events.channelJoined', {
            channel
        }));

        this.app.OnJoin.forEach((value, key) => {
            try {
                value.call.bind(this.app, channel, nick, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'onJoin'
                }), err);
    ...
    handleOnPart(channel, nick, reason, message) {
        ...
        if (nick === this.app.nick) logger.info(t('events.channelParted', {
            channel,
            normalizedReason
        }));

        this.app.OnPart.forEach((value, key) => {
            try {
                value.call.bind(this.app, channel, nick, normalizedReason, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'onPart'
                }), err);
    ...
    handleOnQuit(nick, reason, channels, message) {
        ...
        if (nick === this.app.nick) logger.info(t('events.quitLogging', {
            channels: _.isObject(channels) ? Object.keys(channels).join(', ') : channels,
            normalizedReason
        }));

        this.app.OnQuit.forEach((value, key) => {
            try {
                value.call.bind(this.app, nick, normalizedReason, channels, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'onQuit'
                }), err);
    ...
    handleOnTopic(channel, topic, nick, message) {
        ...
        if (nick === this.app.nick) logger.info(t('events.topicLogging', {
            channel,
            normalizedTopic
        }));

        this.app.OnTopic.forEach((value, key) => {
            try {
                value.call.bind(this.app, channel, normalizedTopic, nick, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
                    command: 'opTopic'
                }), err);
    ...
    handleCtcpCommands(from, to, text, type, message) {
        ...
        this.app.OnCtcp.forEach((value, key) => {
            try {
                value.call.bind(this.app, from, to, normalizedText, type, message)();
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
    ...
    handleRegistered(message) {
        ...
        this.app.Registered.forEach((value, key) => {
            try {
                value.call.bind(this.app, message);
            } catch (err) {
                this.app._errorHandler(t('errors.genericError', {
     ...
     * @param {object} message IRC information such as user, and host
     */
    handleCommands(from, to, text, message) {
        const normalizedText = IrcWrappers._normalizeText(text);
        ...
        const nickMatched = normalizedText.startsWith(this.app._ircClient.nick);
        ...
        if (!is.triggered && !is.ignored && !is.self) {
            this.app.Listeners.forEach((value, key) => {
                try {
                    value.call.bind(this.app, to, from, normalizedText, message, is)();
                } catch (err) {
                    this.app._errorHandler(t('errors.genericError', {
        ...
        if (matchedInvalidCommand) {
            ...
            if(!cmd || _.isEmpty(cmd)) {
                this.app.say(to, `${from}, {${replyOptions.join('|')}}`);
                return false;
            }
            const actualText = `${cmd} ${output}`.trim();
            this.app.say(to, t('errors.invalidCommand', {
                from,
                cmd: actualText
            }));
            return false;
        ...
        if (owner() || guestCommand() || validChannelVoiceUser() || channelOpUser()) {
            try {
                // Call Function
                command.call.bind(this.app, to, from, output, message, is)();
        ...
        // The following administration levels piggy back on services, thus we check the acc status of the account and defer
            ...
            this.app.AdmCallbacks.set(`${from}.${Date.now()}`, {cmd, from, to, text: normalizedText, message, is});
     ...
     */
    handleAuthenticatedCommands(nick, to, text, message) {
        const normalizedText = IrcWrappers._normalizeText(text);
        ...
        const invalidAdmin = () => this.app.Config.accessLevels.admin && !_.includes(this.app.Admins, _.toLower(admCall.from));
        ...
        try {
            ...
            command.call.bind(this.app, admCall.to, admCall.from, output, admCall.message, admCall.is)();
