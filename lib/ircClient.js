...
// TODO Implement
client.cPrivMsg = function(nick, channel, message) {
    if (!this.isOpInChannel(channel) || !this.isInChannel(channel, nick)) this.say(nick, message);
    else this.send('CPRIVMSG', nick, channel, `:${message}`);
    // CPRIVMSG[edit]
    // Syntax:
...
// Every sixty seconds, check to see if we should take back our primary nick
setTimeout(server => {
    // No Nickserv nick No connection, requested disconnect, nick already the same, does not have nickserv password, has empty nickserv password => bail
    if (!_.has(config, 'nickserv.nick') || _.isEmpty(config.nickserv.nick) || client.conn.readyState != 'open' || client.conn.requestedDisconnect || client.nick == config.bot.nick || !_.has(config, 'nickserv.password') || _.isEmpty(config.nickserv.password)) return;
    // If we have flood protection, use the _sendImmediate command to push the commands to the top of the stack
    let _send = client.floodProtection ? client._sendImmediate : send;
    // Build up the nickserv nick
    let nickserv = config.nickserv.nick;
    // if we need to append a chanserv host (dalnet), append it
    if (_.has(config, 'nickserv.host') && _.isString(config.nickserv.host) && !_.isEmpty(config.nickserv.host)) nickserv = nickserv + `@${config.nickserv.host}`;
    // Send ghost to kill any existing nicks
    _send('PRIVMSG', nickserv, `ghost ${config.bot.nick} ${config.nickserv.password}`);
