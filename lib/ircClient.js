...
const originalNick = config.irc.nick;
...
const nickWatcher = scheduler.schedule('maintainPrimaryNick', {
    ...
    if (
        _.isString(config.nickserv.nick) &&
        !_.isEmpty(config.nickserv.nick) &&
        client.conn.readyState == 'open' &&
        !client.conn.requestedDisconnect &&
        _.isString(config.nickserv.password) &&
        !_.isEmpty(config.nickserv.password) &&
        client.nick !== originalNick
    ) {
