...
!function (root, name, definition) {
    ...
    const regexpMatrix = [
        ...
        [ // ch
            ...
            [
                "(.)ture", [
                "$1chure", "$1chr", "$1chur", "$1tshur", "$1tshure"
            ],
                5,
                1
            ]
        ...
        [ // o
            ...
            [
                "([" + sets.c + "])wa([" + sets.c + "])", ["$1wo$2"],
                3,
                1
            ],
            [
                "([" + sets.c + "])wo([" + sets.c + "])", ["$1wa$2"],
                3,
                1
            ],
            [
        ...
        [ // ?
            [
                "([" + sets.c + "])ai([" + sets.c + "])", ["$1ei$2"],
                3,
                1
            ],
            [
                "([" + sets.c + "])ei([" + sets.c + "])", ["$1ai$2"],
                3,
                1
            ],
            [
        ...
        [ // ?
            [
                "([" + sets.c + "])o([" + sets.c + "])([" + sets.v + "])", [
                "$1ou$2$3", "$1ow$2$3", "$1oa$2$3"
            ],
                2,
                1
            ],
    ...
    const obfuscate = function (string) {
        ...
        for (let l = 0; l < lines.length; l++) {
            ...
            const obfuscatedWords = words.map(function (word) {
                let notEarlierThanPosition = -1;
                let nothingMoreToObfuscate = false;
                let atLeastSomethingObfuscated = false;
                while (!nothingMoreToObfuscate) {
                    let somethingToReplaceFound = false;
                    let bestRegexp;
                    let bestPosition = null;
                    let isReplaced = false;
                    const regexpMatrixShuffled = shuffle_array(regexpMatrix);
                    for (let i = 0; i < regexpMatrixShuffled.length; i++) {
                        const regexpSet = shuffle_array(regexpMatrix[i]);
                        for (let j = 0; j < regexpSet.length; j++) {
                            const regexp = regexpSet[j];
                            const searchRegexp = new RegExp(regexp[0], "i");
                            const searchedPosition = word.search(searchRegexp);
                            if (searchedPosition !== -1 // the match actually happened
                            ) {
                                if (searchedPosition > notEarlierThanPosition - regexp[3] // the position is later than the part that we have already processed && (bestPosition === null || searchedPosition < bestPosition) // this is the first position found or the better position found
                                ) {
