...
module.exports = app => {
  ...
  // Update packages
  const updatePackages = pkgManager => new Promise((resolve, reject) => shell.exec(`${pkgManager} install`, execSettings(), (code, stdOut, stdErr) => {
  ...
  const halt = (to, from) => {
    ...
    app._ircClient.disconnect(`${from} has asked me if I could leave for a second and do something important, I shall return`, () => process.exit());
  };
