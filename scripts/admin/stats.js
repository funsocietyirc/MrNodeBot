...
module.exports = app => {
     ...
     * @returns {*}
     */
    const statsHandler = (to, from) => app.Stats.forEach((value, key) => {
     ...
     * @param to
     */
    const uptimeHandler = to => app.say(to, `I have been alive since ${helpers.StartTime}, about ${helpers.StartTime.fromNow()}`);
     ...
     */
    const highLowHandler = async (to, from, text) => {
        try {
            // Extract Results
