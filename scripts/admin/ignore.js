...
module.exports = app => {
    const mute = (to, from, text, message) => {
        ...
        if (!Admins.contains(textArray[0].toLowerCase()) && !app.Ignore.contains(textArray[0].toLowerCase())) {
            app.Bot.say(to, `${textArray[0]} has been muted. May there be peace.`);
            app.Ignore.push(textArray[0].toLowerCase());
            storage.setItemSync('ignore', app.Ignore);
        } else {
            app.Bot.say(to, `${textArray[0].capFirst()} has either already been muted, or is an Administrator and is beyond my control`);
        }
    };
    ...
    const unmute = (to, from, text, message) => {
        let textArray = text.split(' ');
        if (!textArray[0]) {
            app.Bot.say(to, 'You need to specify a user');
        } else {
            if (app.Ignore.contains(textArray[0].toLowerCase())) {
                app.Bot.say(to, `${textArray[0]} has been unmuted`);
                app.Ignore.splice(textArray.indexOf(textArray[0]));
                storage.setItemSync('ignore', app.Ignore);
            } else {
                app.Bot.say(to, `${textArray[0].capFirst()} is not on the mute list`);
            }
        }
    };

    const ignored = (to, from, text, message) => {
        app.Bot.say(to, '--- Ignore list ---');
        app.Ignore.forEach(function(user) {
            app.Bot.say(to, user.capFirst());
        });
        app.Bot.say(to, `For a total of: ${app.Ignore.length}`);
    };
